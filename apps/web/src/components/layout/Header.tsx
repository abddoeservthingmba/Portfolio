  ...
  return (
    ...
    >
      <Container className="flex h-16 items-center justify-between gap-4">
        ...
        >
          <span className="grid h-7 w-7 place-items-center rounded-lg bg-accent text-xs font-bold text-accent-fg transition-transform duration-400 [transition-timing-function:var(--ease-spring)] group-hover:rotate-12 group-hover:scale-110">
            {siteTitle.charAt(0)}
          </span>
        ...
        <nav aria-label="Primary" className="hidden lg:block">
          <ul className="flex items-center gap-0.5 rounded-pill border border-border bg-surface/60 p-1 backdrop-blur">
            {NAV_ITEMS.map((item) => (
              <li key={item.to}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="press flex h-9 items-center rounded-control border border-border bg-surface px-3.5 text-sm text-muted lg:hidden"
          >
            {menuOpen ? 'Close' : 'Menu'}
          </button>
        </div>
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          ...
          isActive
            ? 'bg-accent text-accent-fg shadow-[0_2px_12px_-2px_var(--accent-glow)]'
